...
Конвенция путей: `<slug-поездки>/<исходное-имя-файла>` — имя файла оставляем
как есть при скачивании (менять не нужно), меняется только путь: файл кладём
в "папку" с именем slug'а поездки. В тексте markdown место фото отмечается
`<!-- photo: https://pub-.../<slug>/<имя-файла> -->` — рендерится как `<img>`
